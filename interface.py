...
def interface():
    ...
    image = tk.PhotoImage(file=".\\back\\background.png")
    background_label = tk.Label(text_frame, image=image)
    background_label.place(x=0, y=0, relwidth=1, relheight=1)

    # Создаю метку с текстом в основном окне
    text_label = tk.Label(text_frame, text="Добро пожаловать в Ok-Gosling", fg="black", bg="dim gray",
                          font=("Arial", 14))
    text_label.pack(pady=100, anchor="n")


    listen_button = tk.Button(window, text="Нажмите \n чтобы говорить", command=startListen, width=20, height=2, bg="chartreuse2",
                              fg="black")
